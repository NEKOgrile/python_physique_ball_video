...
class Balle:
    ...
    def check_wall_cercle_collision(self, cercle):
        ...
        contact = distance + self.radius > cercle.radius

        if contact:
            if cercle.is_in_hole(self.pos):
                cercle.broken = True
                return True  # collision réussie : passe dans le trou
            else:
                # Collision avec la paroi du cercle
                if distance == 0:
                ...
                # Corrige pour éviter que la balle reste collée
                epsilon = 0.5  # marge de sécurité
                self.pos -= normal * (overlap + epsilon)

                # Joue le son une seule fois (par exemple, avec un timer ou un flag)
                if hasattr(self, "collision_sound") and self.collision_sound:
                    self.collision_sound.play()

                print("Contact avec le cercle à", cercle.pos)
...
pygame.mixer.init()
collision_sound = pygame.mixer.Sound("collision.wav")  # Mets ici ton fichier .wav
collision_sound.set_volume(0.5)  # Volume entre 0.0 et 1.0
